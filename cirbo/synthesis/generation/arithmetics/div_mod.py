...
def add_div_mod(
    ...
    *,
    ...
    :param zero_div: if true, division by zero maps both quotient and remainder to
        zero; otherwise quotient and remainder are mapped to the dividend.
    :param big_endian: defines how to interpret numbers, big-endian or little-endian
        format
    :param basis: in which basis should generated function lie. Supported [XAIG, AIG].
...
def precompute_unsigned(n: int, d: int) -> tuple[int, int, int]:
    """
...
def add_div_by_const(
    ...
    *,
    ...
    """
    Divides an unsigned number by a non-zero integer constant.
    ...
    :param big_endian: defines how to interpret numbers, big-endian or little-endian
        format
    :param basis: in which basis should generated function lie. Supported [XAIG, AIG].
    :return: A list of gate labels representing the quotient.
    """
    input_labels_a = list(input_labels_a)
    if big_endian:
        input_labels_a.reverse()
    ...
    mul, add_val, shift = precompute_unsigned(n, b)

    out = add_mul_constant(circuit, input_labels_a, mul, basis=basis)
    if add_val != 0:
        add_bits = _constant_to_bits(circuit, out[0], add_val)
        out = add_sum_two_numbers(circuit, out, add_bits, basis=basis)

    out = out[shift:]
    return reverse_if_big_endian(out, big_endian)


def _constant_to_bits(
    circuit: Circuit,
...
def add_mod_by_const(
    ...
    *,
    ...
    """
    Computes the remainder of an unsigned number modulo a non-zero integer constant.
    ...
    :param big_endian: defines how to interpret numbers, big-endian or little-endian
        format
    :param basis: in which basis should generated function lie. Supported [XAIG, AIG].
    :return: A list of gate labels representing the remainder.
    """
    input_labels_a = list(input_labels_a)
    if big_endian:
        input_labels_a.reverse()
    ...
    div = add_div_by_const(circuit, input_labels_a, b, big_endian=False, basis=basis)
    if not div:
        return reverse_if_big_endian(input_labels_a, big_endian)
    sub = add_mul_constant(circuit, div, b, basis=basis)
    res = add_sub_two_numbers(circuit, input_labels_a, sub, basis=basis)
