...
def create_alternative_and_gate() -> Circuit:
    ...
    circuit.emplace_gate("not_x1", NOT, ("x1",))
    circuit.emplace_gate("not_x2", NOT, ("x2",))
    circuit.emplace_gate("or", AND, ("not_x1", "not_x2"))
    circuit.emplace_gate("out", NOT, ("or",))
...
def test_get_by_raw_truth_table_model_returns_minimal_size():
    ...
    retrieved_circuit = db.get_by_raw_truth_table(truth_table)
    assert retrieved_circuit == small_circuit
...
def test_save_and_load_database(basis, use_label):
    ...
    loaded_db = CircuitsDatabase(stream)
