...
from boolean_circuit_tool.synthesis.generation import generate_plus_one, generate_inputs
...
def test_miter():
    for n in range(2, 10):
        ...
        plus_two = Circuit().add_circuit(plus_one, circuit_name='first').extend_circuit(plus_one, circuit_name='second')
        plus_three = Circuit().add_circuit(plus_two).extend_circuit(plus_one, circuit_name='third')
