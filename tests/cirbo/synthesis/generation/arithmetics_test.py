...
@pytest.mark.parametrize("big_endian", [True, False])
@pytest.mark.parametrize("func", [add_sum_n_bits])
def test_add_sum_n_bits(func, basis, n, big_endian):
    ckt = Circuit()
    input_labels = [f'x{i}' for i in range(n)]
    for i in range(n):
        ckt.add_gate(Gate(input_labels[i], INPUT))
...
def test_sub_two_numbers(func, basis, size, big_endian):
    ...
    res = func(
        ckt, input_labels[:x], input_labels[x:], basis=basis, big_endian=big_endian
    )
    ckt.set_outputs(res)
...
def test_sum_weighted_bits_in_xaig_density(n, density_in_percent):
