...
from boolean_circuit_tool.core.python_function import PythonFunction, FunctionType
...
def sum_inputs_with_negations(xs: list[bool], negations: list[bool]) -> tp.Sequence[bool]:
...
@pytest.mark.parametrize("function, input_size, output_size", [
    (f_max, 3, 1),
    (f_max_and_min, 4, 2),
    (f_sum, 3, 2),
])
...
@pytest.mark.parametrize("inputs, value", [
...
@pytest.mark.parametrize("inputs, index, value", [
...
@pytest.mark.parametrize("function, is_constant", [
    (f_const, True),
    (f_max, False),
])
...
@pytest.mark.parametrize("function, output_index, inverse, is_monotonic", [
...
def test_is_monotonic(function: FunctionType, output_index: int, inverse: bool, is_monotonic: bool):
    bf = PythonFunction(function)
    assert bf.is_monotonic_at(output_index, inverse=inverse) == is_monotonic


@pytest.mark.parametrize("function, is_symmetric", [
    (f_max, True),
    (f_sum, True),
...
@pytest.mark.parametrize("negations", [
    [False, False, False],
    [False, False, True],
    [False, True, False],
    [False, True, True],
...
@pytest.mark.parametrize("function, input_index, depends", [
...
def test_is_dependent_on_input_at(function: FunctionType, input_index: int, depends: bool):
    bf = PythonFunction(function)
    assert bf.is_dependent_on_input_at(0, input_index) == depends


@pytest.mark.parametrize("function, input_index, is_equal", [
    (f_arg0, 0, True),
    (f_arg0, 1, False),
    (f_arg0, 2, False),
    (f_not_arg0, 0, False),
])
def test_is_output_equal_to_input(function: FunctionType, input_index: int, is_equal: bool):
    bf = PythonFunction(function)
...
def test_is_output_equal_to_input_negation(function: FunctionType, input_index: int, is_equal: bool):
    bf = PythonFunction(function)
    assert bf.is_output_equal_to_input_negation(0, input_index) == is_equal


@pytest.mark.parametrize("function, significant_inputs", [
    (f_arg0_and_arg1, [0, 1]),
    (f_arg1_or_arg2, [1, 2]),
    (f_arg0_xor_arg2, [0, 2]),
    (f_arg0, [0]),
...
def test_get_significant_inputs_of(function: FunctionType, significant_inputs: list[int]):
    bf = PythonFunction(function)
    assert bf.get_significant_inputs_of(0) == significant_inputs


@pytest.mark.parametrize("function, truth_table", [
