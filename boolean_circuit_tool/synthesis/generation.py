...
def generate_pairwise_if_then_else(n) -> Circuit:
    ...
    return add_pairwise_if_then_else_of(
...
def add_plus_one(circuit: Circuit, result_labels, x_labels) -> Circuit:
    ...
    circuit.add_gate(Gate(result_labels[0], NOT, tuple([x_labels[0]]))).mark_as_output(
        result_labels[0]
    )
    ...
    for i in range(1, out_len):
        if i < inp_len:
            if i < out_len - 1:
                circuit.add_gate(
                    Gate('carry_' + str(i), AND, ('x_' + str(i), 'carry_' + str(i - 1)))
                )
            circuit.add_gate(
                Gate('z_' + str(i), XOR, ('x_' + str(i), 'carry_' + str(i - 1)))
            )
        elif i == inp_len:
...
def add_if_then_else_of(
    circuit, result_label: Label, if_label: Label, then_label: Label, else_label: Label
) -> Circuit:
    (
        ...
        .add_gate(
            Gate(result_label, OR, ('tmp1' + result_label, 'tmp3' + result_label))
        )
...
def add_pairwise_if_then_else_of(
    circuit, result_labels, if_inputs, then_inputs, else_inputs
) -> Circuit:
